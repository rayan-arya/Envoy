...
def build_pipeline(system_prompt, tools=None, transport=None, on_event=None, constraints=None):
    ...
    async def on_client_connected(transport, client):
        logger.info("Client connected - kicking off (Envoy speaks first; opening line via Nemotron)")
        # This is an INSTRUCTION to the model, not Envoy's words. The injector sets the system prompt
        # first, then the LLM generates Envoy's actual opening line from SYSTEM_PROMPT + constraints.
        context.add_message(
            {
                "role": "user",
                "content": (
                    "You're now connected - the other party's line just opened and you hear them "
                    "pick up. Deliver your opening line to start accomplishing your goal."
                ),
            }
        )
